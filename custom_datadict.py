...
preid_reg_list = [r"(?<=Claim Number).*(?=\(0)", r"(?<=Claim Number).*", r"(?<=Preauth Reference no).*",
                  ...
                  r"(?<=AL Number).*", r"(?<=cashless request ID).*?(?=dated)", r"(?<=Preauth ID).*",
                  r"(?<=Incidence no).*", r"(?<=Cashless Authorization no).*(?=under Policy)",
...
amount_reg_list = [r"(?<=Total Authorized amount).*", r"(?<=Approved amount).*", r"(?<=Approved Amount).*",
                   ...
                   r"(?<=Initial Approval Amount).*(?=\()", r"(?<=Total Approved Amount).*",
