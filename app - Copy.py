...
graph_db_location = r"C:\Apache24\htdocs\www\myapp\app\index2\database1.db"
# from apscheduler.schedulers.background import BackgroundScheduler
...
def add1(formparameter):
  ...
  for i in range(0, len(r_credentials)):
    ...
    if (mode == "intervel"):
      ...
      result = []
      with sqlite3.connect(graph_db_location) as con:
        cur = con.cursor()
        q = "select * from graphApi where completed=''"
        cur.execute(q)
        result = cur.fetchall()
...
def process_copy_parallel(result, now, today, row_count_1):
  pool = 4
  size = 4
  b = [result[i:i + (pool * size)] for i in range(0, len(result), pool * size)]
  c = []
  for j in b:
    temp = [j[i:i + size] for i in range(0, len(j), size)]
    params = []
    for i in temp:
      params.append((i, now, today, row_count_1))
    with Pool(size) as p:
        print(p.map_async(process_copy, params))
        p.close()
        p.join()
