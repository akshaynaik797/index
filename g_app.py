  ...
  while ("@odata.nextLink" in syn_data):
    ...
    if 'value' in syn_data:
      for item in syn_data['value']:
        mailList.append(item)
  try:
    with sqlite3.connect("database1.db", timeout=120) as con:
      cur = con.cursor()
      # akshay -> reverse loop
    ...
    subprocess.run(["python", "sms_api.py", "graph api database is locked"])
...
def deltaMessage():
  ...
  try:
    ...
    nexturl = app_config.nexturl #None
          ...
          for item in mailList[::-1]:
            ...
            if 'receivedDateTime' in item:
              try:
                ...
                utc = datetime1.strptime(item['receivedDateTime'], '%Y-%m-%d %H:%M:%S')
                utc = utc.replace(tzinfo=timezone.utc)
            ...
            if data['hasAttachments'] == 1:
              ...
              attach_data = requests.get(getattach, headers={'Authorization': 'Bearer ' + token['access_token'],
                                                      "Prefer": "odata.track-changes",
                                                      },
                                  ).json()
              attach_list = ""

              for i in attach_data['value']:
                ...
                if (fp.find('declar') != -1):
                  continue
                try:
                  if 'contentBytes' in jsondata:
                    temp_var = os.path.splitext(i['name'])[0]
                    temp_var = temp_var.replace('/', '')
                    temp_f_name = 'new_attach/' + temp_var + '_' + str(attach_no) + \
                                  os.path.splitext(i['name'])[1]
            ...
            if flag_a != 'X':
              ...
              try:
                ...
                log_exceptions()
            '''
            if data['hasAttachments'] == 0:
  
              with open('new_attach/email.html', 'w') as tempf:
                tempf.write(data['body']['content'])
              try:
                attach_no = get_fp_seq()
                pdfkit.from_file('new_attach/email.html', 'new_attach/'+str(attach_no)+'_body.pdf', configuration=config)
                attach_list = attach_list + os.path.abspath('new_attach/'+str(attach_no)+'_body.pdf') + ','
              except Exception as e:
                log_exceptions()
            ...
            q1 = f"select * from graphApi where id='{item['id']}'"
            query = f"insert into graphApi values ('{item['id']}', '{subject}', '{l_time}', '{str(datetime.datetime.now())}', '{attach_list}', '', '{sender}')"
            query1 = f"insert into graphApi values ('{item['id']}', '{subject}', '{l_time}', '{str(datetime.datetime.now())}', '{attach_list}', 'A', '{sender}')"
            ...
            try:
              ...
              if cur.fetchall() != []:
                ...
                cur.execute(query) #database is locked, so wait for it....akshay
              db_rows_cnt += 1
            except:
              log_exceptions(query=q) #pass query statement also
              pass
...
def download_pdf_copy(s_r, mail, ins, ct, row_count_1, subject, hid, l_time, files, mail_id, sender):
      ...
      if bool(fp):
        ...
        if (fp.find('declaration_cashless') != -1):
          ...
          if ins == 'Raksha':
            ...
            copyfile('new_attach/'+fp, os.getcwd() + '/' + ins + '/attachments_pdf_' + ct + '/' +fp)
            break
            # fp = open(filepath, 'wb')
  ...
  if (t_p == 0):
    ...
    if 'body.pdf' in body_pdf:
      copyfile(fp, os.getcwd() + '/' + ins + '/attachments_' + ct + '/' + os.path.split(fp)[1])
